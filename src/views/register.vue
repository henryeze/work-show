  ...
  <div class="body">
    ...
    <div class="main">
      ...
      <b-form  id="form">
        ...
        <b-button pill class="mainbtn" @click="signUp()"><span v-if="this.$store.state.process === false">Sign Up </span> <b-spinner v-if="this.$store.state.process == true" type="grow"></b-spinner></b-button>
      </b-form>
    </div>
  </div>
</template>
  ...
  methods: {
    signUp() {
      ...
      auth.createUserWithEmailAndPassword(this.form.email, this.form.password).then(() => {
        this.$store.dispatch("uploadUserData", {
            form, 
            vueApp
        })
      }).catch( err => {
